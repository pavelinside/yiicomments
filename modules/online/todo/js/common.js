...
Date.prototype.userDays = ['Воскресение', 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота'];

Date.prototype.userDay = function() {
	return this.userDays[this.getDay()];
};
...
function fillSel(el, data, strict, dontchange, def, defVal){
	...
	if(oldval==null)
		oldval = ''; 	
	$(el).html(makeOpts(data, strict, def, defVal)).val('').val(defVal ? defVal : oldval);
	if(!dontchange)
		$(el).change();
}
